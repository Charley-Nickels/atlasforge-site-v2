(function () {
  ...
  const normalizeDecisionData = (payload) => {
    if (Array.isArray(payload)) {
      return { meta: {}, blocks: payload };
    }
    if (payload && Array.isArray(payload.blocks)) {
      return { meta: payload.meta || {}, blocks: payload.blocks };
    }
    return { meta: {}, blocks: [] };
  };

  const loadData = () => {
    const paths = ['/data/decision_blocks.json', '../data/decision_blocks.json'];

    const tryFetch = (index = 0) => {
      const url = paths[index];
      if (!url) {
        throw new Error('No decision blocks path available');
      }

      return fetch(url).then((response) => {
        if (!response.ok) {
          throw new Error(`Failed to load decision blocks: ${response.status} (${url})`);
        }
        return response
          .json()
          .then((data) => ({ data, url }))
    ...
    tryFetch()
      .catch(() => tryFetch(1))
      .then(({ data, url }) => {
        dataCache = normalizeDecisionData(data);
        renderBlocks(dataCache);
        setConsoleStatus(`Decision blocks loaded from ${url}.`, false);
      })
      .catch((err) => {
        console.error(err);
