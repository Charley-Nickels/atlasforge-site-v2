...
Phase 5 locks brand canon (roles, tone, visibility), navigation and homepage structure, IP visibility (OIA, Neighborhood Watch, others), governance transparency, and positioning for AtlasStudio and Atlas-V. The console renders the structured blocks from `data/decision_blocks.json`, and `docs/atlasforge_canon_index.md` describes how those decisions feed the future `docs/atlasforge_canon.md`. AtlasStudio Enterprise (see `docs/AtlasStudio_Enterprise.md`) defines a decision-engine pattern for Microsoft 365-style workflows; Phase 5 canon choices influence how it is described and surfaced on marketing pages but do not add backend features yet. Most dropdowns now act as “dials” for visibility, maturity, transparency, and contact openness rather than simple include/exclude toggles.

## 3. Block-by-Block Decision Overview
Below follows each block in the order defined in `data/decision_blocks.json`, with its intent and primary areas affected. Every block now uses multi-level dropdowns as dials (visibility, role importance, maturity, transparency, or contact openness) rather than binary toggles, so choose the level that matches how prominent or ready each element should be.

### 3.1 Core Pillars Visibility
- **Block ID:** core_pillars_visibility — items: 8
- **Purpose:** Sets how AtlasForge pillars (umbrella, Games, Media, Studio/Tools, Atlas-V/Engine, Merch, Labs, Contact) rank across hero, nav, sections, and footer using a visibility dial from hidden to hero treatment.
...
- **Purpose:** Defines ecosystem roles for AtlasForge umbrella, Atlas-V, AtlasStudio, Atlas Engine, Media, Games, SKB/Audio, VoiceForge, Hypercasual Forge, Template Factory, MerchForge, Narrative Labs, Documentary Labs, and Media labeling using a role/importance scale (internal-only through primary driver).
...
- **Purpose:** Chooses public tone/voice per brand and key IP (AtlasForge, Atlas-V, AtlasStudio, Octopus in Action, AtlasForge Media) across a curated tonal set (playful, grounded, technical, experimental, or low-hype/refined).
...
- **Purpose:** Frames Atlas-V identity (quiet utility, partner copilot, governance-forward, headline engine), governance and loop visibility, explanation depth, CTA intensity, and decision-engine references via multi-level visibility/role choices instead of binary mentions.
...
- **Purpose:** Determines AtlasStudio visibility level, product vs prototype framing, depth of UI/mocks, CTA strength, and tab references using dials for prominence, maturity, and detail.
...
- **Purpose:** Controls which pages are present and how they appear (home, Atlas-V, AtlasStudio, OIA, SKB, About/Studio, Media/Ecosystem hubs, roadmap/governance pages), plus grouping under Ecosystem or similar nav hubs, using visibility dials from hidden/footer utility to primary-nav-with-hub.
...
- **Purpose:** Selects which homepage sections exist and their prominence (AtlasForge overview, Atlas-V how-it-works, AtlasStudio teaser, OIA highlight, Labs/Under Construction, governance blurb, contact/work-with-us, and other featured blocks) using a visibility scale from hidden to hero-level placement.
...
- **Purpose:** Approves or backlogs sub-labels like Hypercasual Forge, Template Factory, MerchForge, Narrative Labs, Documentary Labs, Labs variants, and SKB/VoiceForge positioning using maturity/visibility blends (internal-only, concept mention, low-key, fully branded, or legacy).
...
- **Purpose:** Sets visibility/status for Octopus in Action, Neighborhood Watch, and other named IPs; defines how OIA appears (hero/section/nav/footer) and whether Neighborhood Watch is mentioned, using status dials (idea → in development → live) plus visibility and promo depth controls.
...
- **Purpose:** Approves media lanes (true crime/mystery, educational/STEM, commentary/essays, devlogs/behind-the-scenes), decides grouping under AtlasForge Media versus sub-labels, cadence expectations, and safety/merch linkage through maturity and visibility scales (internal-only to active and visible lanes).
...
- **Purpose:** Sets governance/safety transparency level, presence of governance page/sections, decision-engine references, and guardrail acknowledgments using transparency dials from internal-only to deep public detail.
...
- **Purpose:** Configures contact/hiring/partnership touchpoints, inquiry filters, community channels, privacy positioning, and CTA prominence using openness scales (no CTA through actively seeking outreach).
...
- Treat each dropdown as a dial for visibility, maturity/status, transparency, and contact openness; pick the level that fits what should be public today versus staged for later.
