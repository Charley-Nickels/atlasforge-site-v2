...
function bindGhostPanel() {
  const toggle = qs("[data-ghost-toggle]");
  const panel = qs(".as-ghost-panel");
  if (!toggle || !panel) return;

  const applyState = () => {
    panel.classList.toggle("is-open", state.ghostOpen);
    toggle.classList.toggle("is-active", state.ghostOpen);
  };

  toggle.addEventListener("click", () => {
    state.ghostOpen = !state.ghostOpen;
    applyState();
    logMessage(`[Ghost] ${state.ghostOpen ? "Opened" : "Closed"} sidecar.`, { group: "ghost" });
  });
...
function attachButtonRipples() {
  const buttons = qsa(".as-workstation .af-btn");
  buttons.forEach((btn) => {
...
function bindParallax() {
  ...
  shell.addEventListener("mousemove", (event) => {
    ...
    const offsetX = Math.max(Math.min(normX, 1), -1) * 8;
    const offsetY = Math.max(Math.min(normY, 1), -1) * 8;
    shell.style.setProperty("--as-parallax-x", `${offsetX}px`);
    shell.style.setProperty("--as-parallax-y", `${offsetY}px`);
  });
}
...
// ───────────────────────────────────────────────
// AtlasStudio UI Wiring (Prototype)
// ───────────────────────────────────────────────

function initPillToggleGroups() {
  const groups = document.querySelectorAll('.js-pill-group');
  groups.forEach(group => {
    group.addEventListener('click', (evt) => {
      const btn = evt.target.closest('.as-pill-toggle');
      if (!btn) return;
      group.querySelectorAll('.as-pill-toggle').forEach(b => {
...
function initDropdowns() {
  document.addEventListener('click', (evt) => {
    ...
    document.querySelectorAll('.as-dropdown-menu').forEach(menu => {
...
function initCodexGhostPanel() {
  const toggleBtn = document.querySelector('.js-ghost-toggle');
  const panel = document.querySelector('.as-ghost-panel');
  ...
  // Tabs
  const tabs = panel.querySelectorAll('.as-ghost-tab');
  const views = panel.querySelectorAll('.as-ghost-view');
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      views.forEach(view => {
        if (view.getAttribute('data-ghost-view') === target) {
          view.classList.add('as-ghost-view--active');
        } else {
          view.classList.remove('as-ghost-view--active');
        }
      });
    });
  });
...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    console.warn('[AtlasStudio] UI init error:', err);
  }
});
